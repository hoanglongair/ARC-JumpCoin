...
function saveScore() {
    window.parent.postMessage({ type: "ARC_JUMP_GAME_OVER", score }, window.location.origin);
}

function requestPaidStart() {
    window.parent.postMessage({ type: "ARC_JUMP_PLAY_REQUEST" }, window.location.origin);
}
...
window.addEventListener("message", function (event) {
    ...
    if (event.data?.type === "ARC_JUMP_START_GAME") {
        startGame();
    }
});
...
function init() {
    ...
    function gameOver() {
        ...
        if (player.y > height / 2 && flag === 0) {
            ...
            tweet.href = `https://x.com/share?text=I just scored ${score} points in the ARC JumpCoin! @0xbescient`;

            // Gọi lưu điểm + tải bảng xếp hạng
            saveScore();
        }
    }
...
btnLeft.addEventListener("touchstart", () => (player.isMovingLeft = true));
btnLeft.addEventListener("touchend", () => (player.isMovingLeft = false));

btnRight.addEventListener("touchstart", () => (player.isMovingRight = true));
btnRight.addEventListener("touchend", () => (player.isMovingRight = false));
